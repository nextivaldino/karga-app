...
import { CaretDown as ChevronDown, ArrowsInLineVertical as FoldVertical, EnvelopeSimple as Mail, ChatCircle as MessageCircle, PencilSimple as Pencil, PaperPlaneTilt as Send, ShareNetwork as Share2, Trash as Trash2, ArrowsOutLineVertical as UnfoldVertical, UserCircle as UserRound } from '@phosphor-icons/react';
...
import { ESTADO_ICON, ESTADO_LABEL, formatMoeda, type EstadoListaCarga } from '@/lib/cargaEstado';
import { corAcento, corSuave, corTextoSobre } from '@/lib/rowAccents';
...
import type { CargaPendente, ContentorDisponivel, NovaCargaPendenteInput } from '@/types';
...
function BotaoEnviarContacto({ telefone, email, corTexto }: { telefone: string | null; email: string | null; corTexto: string }): React.JSX.Element {
  ...
  return (
    <div ref={ref} className={`relative flex h-9 ${LARGURA_ACAO} shrink-0 items-center justify-center`}>
      <button
        ...
        style={{ color: corTexto, opacity: disponivel ? 1 : 0.3 }}
        className="flex h-9 w-9 items-center justify-center rounded-control active:opacity-70"
      >
        <Share2 size={19} />
      </button>
      {aberto ? (
...
function GrupoContactoHeader({
  ...
  return (
    <div className="relative z-10 flex w-full items-center gap-2 px-4 py-2.5" style={{ backgroundColor: cor }}>
      <button type="button" onClick={onToggle} className="flex min-w-0 flex-1 items-center gap-2 text-left">
        ...
        <span
          ...
          style={{ backgroundColor: corTexto === '#ffffff' ? 'rgba(255,255,255,0.22)' : 'rgba(0,0,0,0.12)', color: corTexto }}
        >
          <UserRound size={20} />
        </span>
        <span className="min-w-0 flex-1 truncate text-[16px] font-semibold" style={{ color: corTexto }}>
          {grupo.label}
        </span>
        <span className="shrink-0 text-[11px] font-medium" style={{ color: corTexto, opacity: 0.75 }}>
          {grupo.itens.length} {grupo.itens.length === 1 ? 'carga' : 'cargas'}
        </span>
        <EstadoIcon size={14} className="shrink-0" style={{ color: corTexto }} aria-label={ESTADO_LABEL[estado]} />
      </button>
      <span className={`${LARGURA_VALOR} shrink-0 text-right text-[13px] font-semibold tabular-nums`} style={{ color: corTexto }}>
        {formatMoeda(totalValor)}
      </span>
      <BotaoEnviarContacto telefone={contacto.telefone} email={contacto.email} corTexto={corTexto} />
    </div>
  );
}
...
export function CargasPage(): React.JSX.Element {
  ...
  const { vista, filtro } = useCargasToolbar();
  ...
  const corPorContacto = useMemo(() => {
    ...
    grupos.forEach((g, i) => mapa.set(g.chave, corSuave(corAcento(i, theme))));
    return mapa;
  }, [grupos, theme]);
      ...
      <div className="flex flex-col gap-3 py-4">
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-3 px-4">
            {filtradas.map((l) => (
              <CargaGridCard
                key={l.id}
                linha={l}
                cor={corPorContacto.get(l.emissorNome.trim().toLowerCase()) ?? corSuave(corAcento(0, theme))}
                acoes={acoesPara(l)}
              />
            ))}
          </div>
          ...
          <div className="flex flex-col gap-2.5">
            {/* Alternar todos vive junto ao cabeçalho de colunas que afeta —
                deixou de disputar espaço com os controlos de vista/filtro,
                agora fundidos na ilha dinâmica do cabeçalho global. */}
            ...
            <CargaListHeader />
            {/* Blocos de cor vivos: cada contacto é uma faixa sólida da sua
                cor (não um tint a 15%), encostada à seguinte — o próprio
                limite de cor separa um grupo do outro, sem cinzento neutro
                a meio. Nada muda no alinhamento de colunas/menus de ação
                por dentro do grupo, só o preenchimento passa a ser cheio. */}
            <div className="flex flex-col overflow-hidden">
              {grupos.map((grupo, grupoIndex) => {
                const cor = corSuave(corAcento(grupoIndex, theme));
                ...
                return (
                  <div key={grupo.chave} className="flex flex-col">
                    ...
                    {expandido
                      ? grupo.itens.map((l) => <CargaListRow key={l.id} linha={l} corGrupo={cor} corTexto={corTexto} acoes={acoesPara(l)} />)
                      : null}
                  </div>
                );
              })}
